...
def _holt_win_add_mul_dam(x, xi, p, y, l, b, s, m, n, max_seen):
    ...
    for i in range(1, n):
        ...
        b[i] = (beta * (l[i] - l[i - 1])) + (betac * phi * b[i - 1])
        s[i + m - 1] = (y_gamma[i - 1] / (l[i - 1] + phi * b[i - 1])) + (
            gammac * s[i - 1]
        )
...
def _holt_win_mul_mul_dam(x, xi, p, y, l, b, s, m, n, max_seen):
    ...
    for i in range(1, n):
        ...
        b[i] = (beta * (l[i] / l[i - 1])) + (betac * b[i - 1] ** phi)
        s[i + m - 1] = (y_gamma[i - 1] / (l[i - 1] * b[i - 1] ** phi)) + (
            gammac * s[i - 1]
        )
...
def _holt_win_add_add_dam(x, xi, p, y, l, b, s, m, n, max_seen):
    ...
    for i in range(1, n):
        ...
        b[i] = (beta * (l[i] - l[i - 1])) + (betac * phi * b[i - 1])
        s[i + m - 1] = (
            y_gamma[i - 1] - (gamma * (l[i - 1] + phi * b[i - 1])) + (gammac * s[i - 1])
        )
    return sqeuclidean((l + phi * b) + s[: -(m - 1)], y)
...
class ExponentialSmoothing(TimeSeriesModel):
    ...
    def __init__(
        ...
        if (self.trend == "mul" or self.seasonal == "mul") and np.any(
            [x <= 0.0 for x in endog]
        ):
            raise NotImplementedError("Unable to correct for negative or zero values")
    ...
    def fit(
        ...
        if optimized:
            ...
            init_beta = (
                beta if beta is not None else 0.1 * init_alpha if trending else beta
            )
            init_gamma = None
            ...
            if seasoning:
                ...
                xi = np.array(
                    [alpha is None, False, False, True, False, False] + [False] * m
                )
            ...
            if use_basinhopping:
                ...
                res = basinhopping(
                    ...
                    p[xi],
                    minimizer_kwargs={
                        "args": (xi, p, y, l, b, s, m, self.nobs, max_seen),
                        "bounds": bounds[xi],
                    },
                    stepsize=0.01,
                )
...
class SimpleExpSmoothing(ExponentialSmoothing):
    ...
    def fit(self, smoothing_level=None, optimized=True):
        ...
        return super(SimpleExpSmoothing, self).fit(
            smoothing_level=smoothing_level, optimized=optimized
        )
